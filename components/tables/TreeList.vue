<template>
    <div v-if="treeList.length > 0">
        <div v-for="node in  childList" :key="node.id" class="mx-4">
            <input type="checkbox" id="checkBox" v-if="checkBox" :checked="props.defaultCheckBox.includes(node.id)  ? true : false" @change="handleChildCheckBox(node.id)"/>
            {{ node.label}}
        <div v-if="node.children" class="mx-3">
            <!-- <Tree :data="node.children" :new_parent="true"/> -->
        </div>
    </div>
    <span  class="mx-4" v-if="seeall" @click="handleSeeAll">seeall</span>
    </div>
</template>
