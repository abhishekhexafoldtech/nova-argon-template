  ...
  <section class="edit_mang_wrap">
    <div class="mang_inner">
      ...
      <div class="otp_wrap">
        ...
        <div class="fieldrow">
          <el-input v-for="(value, index) in mobileOtp" :key="index" :ref="`mobileOtpInput${index}`"
            v-model="mobileOtp[index]" maxlength="1" @input="onMobileOtpInput(index)"
            @keydown="onMobileOtpKeyDown(index, $event)"></el-input>
        </div>

        <p class="form_text mb-5">
          <nuxt-link to="/">Didn't receive OTP code?</nuxt-link> Resend in {{ minutes }}:{{ seconds < 10 ? "0" + seconds :
            seconds }}</p>

            <p class="form_text">Please enter the 6-digit OTP sent to your email</p>

            <div class="fieldrow">
              <el-input v-for="(value, index) in emailOtp" :key="index" :ref="`emailOtpInput${index}`"
                v-model="emailOtp[index]" maxlength="1" @input="onEmailOtpInput(index)"
                @keydown="onEmailOtpKeyDown(index, $event)"></el-input>
            </div>
